...
class AlarmDataService{
    ...
    func getAlarm(completion: @escaping (NetworkResult<AlarmResponse>) -> Void){
        ...
        // 통신 성공했는지에 대한 여부
        dataRequest.validate().responseDecodable(of: AlarmResponse.self) { dataResponse in
    ...
    private func judgeStatus(by statusCode: Int, _ data: AlarmResponse) -> NetworkResult<AlarmResponse> {
        switch statusCode{
