...
class PreviewAlarmDataService{
    ...
    func getTagPreview(tagId: Int, completion: @escaping (NetworkResult<PreviewAlarmResponse>) -> Void){
        ...
        // 통신 성공했는지에 대한 여부
        dataRequest.validate().responseDecodable(of: PreviewAlarmResponse.self) { dataResponse in
    ...
    private func judgeStatus(by statusCode: Int, _ data: PreviewAlarmResponse) -> NetworkResult<PreviewAlarmResponse> {
        switch statusCode{
